...
void setUp(void) {
  ...
  Scheduler *sched = create_fcfs_scheduler();
...
void test_process_blocking_and_unblocking(void) {
  ...
  // Tick 4 -> 5: P1 em IO (falta 0), vira READY.
  // CPU livre puxa P1. P1 tem burst de 1, então ele roda e FINALIZA no mesmo
  // tick!
  core_tick(&core);
...
void test_process_finish(void) {
  ...
  TEST_ASSERT_EQUAL(STATE_FINISHED, processos[0]->state);
  TEST_ASSERT_EQUAL_INT(4, processos[0]->finish_time);
